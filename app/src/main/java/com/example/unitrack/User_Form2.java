...
public class User_Form2 extends AppCompatActivity {

    EditText editStudentID, editStudentAdd, editStudentName;
    DatabaseReference userRef;
    TextView tvCopyOfGrades;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tvCopyOfGrades = findViewById(R.id.TVCopyOfGrades);

        userRef = FirebaseDatabase.getInstance().getReference().child("Request");

    }
    public void Back(View view) {
        Intent intent = new Intent(User_Form2.this, User_Dashboard.class);
        startActivity(intent);
        overridePendingTransition(0, 0);
    }

    public void Setting(View view) {
        Intent intent = new Intent(User_Form2.this, Setting.class);
        startActivity(intent);
        overridePendingTransition(0, 0);
    }
    ...
    private void ConfirmRequest() {
        ...
        String document = tvCopyOfGrades.getText().toString();
        ...
        userRef.push().setValue(userInformation).addOnCompleteListener(new OnCompleteListener<Void>() {
            ...
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()){
                    Toast.makeText(User_Form2.this, "Successfully Submitted", Toast.LENGTH_SHORT).show();
                    editStudentID.getText().clear();
                    editStudentAdd.getText().clear();
                    editStudentName.getText().clear();
                }else{
                    Toast.makeText(User_Form2.this, "Failed to Submit" + task.getException().getMessage(), Toast.LENGTH_SHORT).show();
                }
            }
        });
